		...
		if($isUpload == true)
		{
			...
			if($statString[$status] != "Accepted")
			{
				...
				else
				{
					...
					//----Invoke online judge-----------
					#echo "./onj $destFile $file $timeLimit";
					#exec("./onj $destFile $file $timeLimit", $output, $verdict);		
					$exec="'$destFile $file $timeLimit'";
					...
					$t = time();
					$verdict=100;
					$query = "insert into `submissions` (userid, problemid, status, time, src_cd, timelimit) values($userid, $file, $verdict, $t,'$destFile', $timeLimit)";
					#echo "QUERY $query";
					mysql_query($query);
